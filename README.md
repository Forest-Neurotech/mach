...
- 🧪 Differentiable beamforming for PyTorch (`mach.autograd`, `pip install mach-beamform[torch]`): gradients to the channel data (adjoint), transmit arrivals, coordinates, sound speed, start time and per-element receive delays (experimental, see [BENCHMARKS.md](BENCHMARKS.md))
