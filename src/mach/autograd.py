...
class _BeamformIQ(torch.autograd.Function):
    ...
    def forward(  # type: ignore[override]
        ...
        ctx.save_for_backward(channel_data, rx_coords_m, scan_coords_m, tx_wave_arrivals_s)
    ...
    def backward(ctx, grad_out):  # type: ignore[override]
        channel_data, rx_coords_m, scan_coords_m, tx_wave_arrivals_s = ctx.saved_tensors
        ...
        grad_c = torch.zeros(1, dtype=torch.float64, device=device) if need[4] else None
        grad_t0 = torch.zeros(1, dtype=torch.float64, device=device) if need[5] else None

        if any(g is not None for g in (grad_channel, grad_rx, grad_scan, grad_tx, grad_c, grad_t0)):
...
def beamform(
    ...
    *,
    ...
    ``n_frames`` defaults to ``channel_data.shape[2]`` and may be smaller when the frame axis
    is padded (see the module docstring).
    """
    if n_frames is None:
